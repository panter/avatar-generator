...
export const getPolygon = ({ shapeId, position, rotation, backface }) => {
...
export default ({ avatar, shapeId, blackList = [], newPosition = null, newRotation = null, newFace = null }) => {
  // check overlap
  ...
  const thisPolygon = getPolygon({ shapeId, position, rotation, backface });

  const polygons = mapValues(
    omit(avatar.shapes, [...blackList, shapeId]),
    ({ position: p, rotation: r, backface: b }, key) => getPolygon({ shapeId: key, position: p, rotation: r, backface: b })
  );
